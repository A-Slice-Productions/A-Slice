...
public var optimisedMode:Bool = ClientPrefs.data.optimisedMode;
function applyOptimisedModeVisibility():Void
{
if (!ClientPrefs.data.optimisedMode) return;
if (camGame != null) camGame.bgColor = FlxColor.BLACK;
if (camHUD != null) camHUD.bgColor = FlxColor.BLACK;
if (boyfriendGroup != null) boyfriendGroup.visible = false;
if (dadGroup != null) dadGroup.visible = false;
if (gfGroup != null) gfGroup.visible = false;
var keepVisible:Array<FlxBasic> = [];
if (uiGroup != null) keepVisible.push(uiGroup);
if (notesGroup != null) keepVisible.push(notesGroup);
if (popUpGroup != null) keepVisible.push(popUpGroup);
for (basic in members)
{
if (basic == null || basic == this) continue;
if (keepVisible.contains(basic)) continue;
if (Std.isOfType(basic, FlxSprite) || Std.isOfType(basic, FlxSpriteGroup) || Std.isOfType(basic, FlxText))
basic.visible = false;
}
}
...
var aggressiveNoteCulling:Bool = ClientPrefs.data.aggressiveNoteCulling;
var heavyChartMode:Bool = ClientPrefs.data.heavyChartMode;
var notesDensityHandling:Bool = ClientPrefs.data.notesDensityHandling;
...
if (ClientPrefs.data.optimisedMode) applyOptimisedModeVisibility();
if (ClientPrefs.data.timeBarType == 'Song Name')
{
timeTxt.size = 24;
timeTxt.y += 3;
...
function extractSpamData(note:SwagNote):Array<Float> {
var noteType:Dynamic = note.noteType;
if (Std.isOfType(noteType, Array)) return cast noteType;
if (noteType != null && noteType.cmpSpam != null) {
var bd = noteType.cmpSpam;
if (Std.isOfType(bd, Array)) return bd;
}
var cmpSpam:Dynamic = note.cmpSpam;
if (Std.isOfType(cmpSpam, Array)) return cast cmpSpam;
if (cmpSpam != null && cmpSpam.cmpSpam != null) {
var bd2 = cmpSpam.cmpSpam;
if (Std.isOfType(bd2, Array)) return bd2;
}
return null;
}
for (section in sectionsData)
...
{
strumTime = songNotes.strumTime;
chartNoteData = songNotes.noteData;
...
if (Std.isOfType(songNotes.noteType, String))
swagNote.noteType = songNotes.noteType;
if (!worldRecordMode) {
burst = extractSpamData(songNotes);
if (burst != null) swagNote.cmpSpam = burst;
}
swagNote.noteData |= gottaHitNote ? 1<<8 : 0; // mustHit
var noteType:Dynamic = songNotes.noteType;
swagNote.noteData |= (section.gfSection && (gfSide ? gottaHitNote : !gottaHitNote)) || noteType == 'GF Sing' || noteType == 4 ? 1<<11 : 0; // gfNote
swagNote.noteData |= (section.altAnim || (noteType == 'Alt Animation' || noteType == 1)) ? 1<<12 : 0; // altAnim
swagNote.noteData |= (noteType == 'No Animation' || noteType == 5) ? 1<<13 : 0; // noAnimation & noMissAnimaiton
unspawnNotes[cnt] = swagNote;
if (songNotes.sustainLength > 0.0)
{
swagNote.holdLength = songNotes.sustainLength;
curStepCrochet = 15000 / daBpm;
roundSus = Math.round(swagNote.holdLength / curStepCrochet);
if (roundSus > 0)
{
var susEnd:Int = roundSus;
var susTime:Float = swagNote.strumTime;
var baseNoteData:Int = swagNote.noteData;
var susNoteType:Dynamic = swagNote.noteType;
for (susNote in 0...roundSus + 1)
{
sustainNote = {
strumTime: susTime + curStepCrochet * susNote,
noteData: baseNoteData,
noteType: susNoteType
};
if (!Math.isNaN(swagNote.density)) sustainNote.density = swagNote.density;
sustainNote.noteData |= 1<<9; // isHold
sustainNote.noteData |= susNote == susEnd ? 1<<10 : 0; // isHoldEnd
...
showProgress();
// Yield to GC periodically for large charts to prevent crashes
if (cnt % 10000 == 0 && cnt > 0 && !ClientPrefs.data.disableGC) {
#if sys
cpp.vm.Gc.run(true);
#end
}
...
var sustainArray:Array<CastNote> = unspawnSustainNotes;
var mergeIdx:Int = cnt;
var sustainLen:Int = sustainArray.length;
while (mergeIdx < sustainLen + cnt) {
unspawnNotes[mergeIdx] = sustainArray[mergeIdx - cnt];
mergeIdx++;
}
unspawnNotes.resize(cnt + sustainLen);
sustainArray.resize(0);
Eseq.pln('Sorting Notes...');
// For charts with 100k+ notes, yield to GC after sorting large arrays
ArraySort.sort(unspawnNotes, sortByTime);
#if sys
if (unspawnNotes.length > 100000 && !ClientPrefs.data.disableGC) {
cpp.vm.Gc.run(true);
}
#end
} else {
...
var baseTime = spawnTime / songSpeed;
if (notesDensityHandling && baseTime > 300) {
var effectiveNps = notes.length * 0.001;
if (effectiveNps > 10000) baseTime = Math.min(baseTime, 300);
else if (effectiveNps > 50000) baseTime = Math.min(baseTime, 150);
else if (effectiveNps > 100000) baseTime = Math.min(baseTime, 100);
}
shownTime = showNotes ? castHold ? Math.max(baseTime, globalElapsed * 1000) : baseTime : 0;
...
while (isDisplay && limitCount < 1000)
{
...
++processedReal;
var noteTimeDelta:Float = Conductor.songPosition - daNote.strumTime;
var shouldProcessNote:Bool = true;
if (aggressiveNoteCulling) {
var heavyWindowMs:Float = songSpeed <= 2.5 ? 1400 : 2200;
if (notes.length > 20000) heavyWindowMs = 1000;
shouldProcessNote = daNote.visible || Math.abs(noteTimeDelta) <= heavyWindowMs;
}
if (!shouldProcessNote) return;
canBeHit = noteTimeDelta > 0;
tooLate = noteTimeDelta > noteKillOffset;
if (tooLate) {
...
dist[availNoteData] = 0.45 * noteTimeDelta * songSpeed;
if (lastSongSpeed != songSpeed) {
...
daNote.followStrumNote(songSpeed, 0.45 * noteTimeDelta * songSpeed); ++shownCnt;
}
if (canBeHit) {
if (daNote.mustPress) {
...
noteTimeDelta > Conductor.stepCrochet) noteMiss(daNote);
}
