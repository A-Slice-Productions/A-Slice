...
class VSlice
{
	...
	public static function convertToPsych(chart:VSliceChart, metadata:VSliceMetadata):PsychPackage
	{
		...
		for (diff in metadata.playData.difficulties)
		{
			...
			Reflect.setField(swagSong, 'generatedBy', 'A-Slice v${MainMenuState.hrkVersion} - PE v${MainMenuState.psychEngineVersion} - Chart Editor V-Slice Importer');
	...
	public static function export(songData:SwagSong, ?difficultyName:String = null):VSlicePackage
	{
		...
		var generatedBy:String = 'A-Slice v${MainMenuState.hrkVersion} - PE v${MainMenuState.psychEngineVersion} - Chart Editor V-Slice Exporter';
		var timeChanges:Array<VSliceTimeChange> = [];
