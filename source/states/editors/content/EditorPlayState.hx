...
class EditorPlayState extends MusicBeatSubstate
{
	...
	var keysArray:Array<Array<String>> = [
		['note_1'],
		['note_2a', 'note_2b'],
	...
	override function update(elapsed:Float)
	{
		...
		if(notes.length > 0)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				daNote.strum = strum;
				daNote.followStrumNote(songSpeed / playbackRate, 0.45 * (Conductor.songPosition - daNote.strumTime) * (songSpeed / playbackRate) * daNote.multSpeed);

				if(!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
					opponentNoteHit(daNote);

				if(daNote.isSustainNote && strum.sustainReduce) daNote.clipToStrumNote();
